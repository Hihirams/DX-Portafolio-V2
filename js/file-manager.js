...
class FileManager {
    ...
    async transferProject(fromUserId, toUserId, projectId) {
        if (!this.isElectron) {
        ...
        try {
            console.log(`🔄 Transfiriendo proyecto ${projectId} de ${fromUserId} a ${toUserId}...`);
            const result = await this.api.transferProject(fromUserId, toUserId, projectId);

            if (result.success) {
                console.log(`✅ Proyecto ${projectId} transferido correctamente`);
                return true;
            } else {
                console.error('❌ Error transfiriendo proyecto:', result.error);
                return false;
            }
        } catch (error) {
            console.error('❌ Error en transferProject:', error);
            return false;
        }
    }
